local Class = class.NewClass("UIElement", "RectTransform")

function Class:New()
	RectTransform.New(self)
...
function Class:Update()
	if self.visible then
		self.rects.global.x = self.x
		self.rects.global.y = self.y
		self.rects.global.w = self.w
		self.rects.global.h = self.h
		
		if self.parent then
			self.rects.global.x = self.parent.rects.padding.x + self.x
			self.rects.global.y = self.parent.rects.padding.y + self.y
		end
